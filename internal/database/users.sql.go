...
const updateUserRecords = `-- name: UpdateUserRecords :one
UPDATE users
...
func (q *Queries) UpdateUserRecords(ctx context.Context, arg UpdateUserRecordsParams) (User, error) {
	row := q.db.QueryRowContext(ctx, updateUserRecords,
		arg.ID,
		arg.Email,
		arg.HashedPassword,
		arg.UpdatedAt,
...
func (q *Queries) UpgradeUserChirpyRedStatus(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRowContext(ctx, upgradeUserChirpyRedStatus, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
